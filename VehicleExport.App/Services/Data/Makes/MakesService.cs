...
{
    public class MakesService : EntityReadService<Makes, string>
    {
        public MakesService(ApplicationDbContext dbContext, IConfiguration configuration, UserManager<ApplicationUser> userManager, ILogger<MakesService> logger) : base(dbContext, configuration, userManager, logger)
        {
        }

        protected override async Task<IQueryable<Makes>> ApplyIdFilter(IQueryable<Makes> queryable, string make)
        {
            return queryable.Where(x => x.Make == make);
        }
